...
def plot_loss_and_metrics(history, plot_loss_only= False, metrics=['acc']):
    ...
    plt.savefig('slp-loss-and-accuracy-metrics.png')
    plt.show()
...
def report_metrics(y_test, y_pred, y_pred_proba, classes, multiclass=False):
    ...
    plt.savefig('slp-confusion-matrix.png')
    plt.show()
...
X_train, y_train, X_test, y_test, IMG_HEIGHT, IMG_WIDTH = load_data(tensorflow.keras.datasets.fashion_mnist, first_layer="dense", channels=1, plot_images=True, class_names=CLASS_NAMES)
...
def build_slp_model(height, width, nb_classes, metrics):
    ...
    model.add(tensorflow.keras.layers.Dense(nb_classes, input_dim=(height * width), use_bias=False, activation='softmax')) # no hidden layers, all inputs connected to all outputs
...
y_pred = np.argmax(model_slp.predict(X_new), axis=1)
